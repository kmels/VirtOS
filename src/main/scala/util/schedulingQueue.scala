...
class schedulingQueue(id:Int,name:String,typeOfQueue:String,quantum:Int,preemptive:Boolean,queueOnQuantumEnd:Int,queueOnIOEnd:Int){
  var pQueue = new scala.collection.mutable.PriorityQueue[Task]()(Ordering.fromLessThan(_.priority < _.priority))
  var sjfQueue = new scala.collection.mutable.PriorityQueue[Task]()(Ordering.fromLessThan(_.burstTime < _.burstTime))
  var fcfsQueue = new scala.collection.mutable.Queue[Task]()

  var timeInExecution:Float = 0
  
  var Queue:Seq[Task] = typeOfQueue.toUpperCase match{
    case "FCFS" => fcfsQueue
  ...
  def apply(index:Int):Task = Queue(index)
